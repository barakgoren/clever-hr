...
import { FileText, TrendingUp, Clock, Users, ArrowRight, Download } from 'lucide-react';
...
function buildMonthlyData(applications: ApplicationWithRelations[]) {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const inMonth = applications.filter((a) => getMonthKey(a.createdAt) === key);
    months.push({
      month: MONTHS[d.getMonth()],
      applications: inMonth.length,
      interviews: inMonth.filter((a) => a.currentStage !== null).length,
    });
  }
  return months;
}
...
function buildTrendData(applications: ApplicationWithRelations[]) {
  ...
  return Array.from({ length: 6 }, (_, i) => {
    ...
    const key = `${d.getFullYear()}-${d.getMonth()}`;
    const count = applications.filter((a) => getMonthKey(a.createdAt) === key).length;
      ...
      <CardContent className="pt-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1.5 text-3xl font-bold text-[var(--color-text-primary)]">{value}</p>
...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const newThisWeek = applications.filter((a) => new Date(a.createdAt) >= weekAgo).length;
    const pendingReview = applications.filter((a) => a.currentStageId === null).length;
    const prevMonthCount = applications.filter((a) => {
      const d = new Date(a.createdAt);
      return d < monthAgo && d >= new Date(now.getTime() - 60 * 24 * 60 * 60 * 1000);
    }).length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <StatCard
          ...
          sub={stats.prevMonthCount > 0 ? `+${Math.round((stats.total / Math.max(stats.prevMonthCount, 1) - 1) * 100)}% from last month` : 'All time'}
          icon={FileText}
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-[var(--color-text-muted)]">Monthly applications vs. interviews</p>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={monthlyData} barGap={4}>
                ...
                <Tooltip
                  ...
                  cursor={{ fill: '#f8fafc' }}
                />
                <Bar dataKey="applications" fill="#4f46e5" radius={[3, 3, 0, 0]} name="Applications" />
                <Bar dataKey="interviews" fill="#a5b4fc" radius={[3, 3, 0, 0]} name="Interviews" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-[var(--color-text-muted)]">Applications received over time</p>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between pb-3">
          ...
          <Link href="/dashboard/applications">
            <Button variant="ghost" size="sm" className="gap-1 text-[var(--color-brand-600)]">
        ...
        <CardContent className="px-0 pb-0">
          {recentApplications.length === 0 ? (
            <p className="px-5 pb-5 text-sm text-[var(--color-text-muted)]">No applications yet.</p>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      <span className="text-xs text-[var(--color-text-muted)]">{formatDate(app.createdAt)}</span>
                      {app.currentStage ? (
                        <span
